...
export const updateProfile = async (req, res) => {
    try {
        let { firstName, lastName, userName, headline, gender, location } = req.body;
        ...
        let user = await User.findByIdAndUpdate(req.userId, {
            firstName, lastName, userName, headline, location, skills, education, experience, profileImage, coverImage, gender
...
export const search = async (req, res) => {
    try {
        ...
        console.log(error);
        return res.status(200).json({ message: `search error: ${error}`});
    }
}
...
export const getSuggestedUser = async (req, res) => {
    try {
        ...
        console.log(error);
        return res.status(200).json({ message: `getSuggestedUser error: ${error}`});
    }
}