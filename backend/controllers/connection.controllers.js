...
export const sendConnection = async (req, res) => {
    try {
        ...
        let existingConection = await Connection.findOne({
            sender,
            receiver: id,
            status: "pending"
        })
        ...
        if (senderSocketId) {
            io.to(senderSocketId).emit("statusUpdate", { updatedUserId: id, newStatus: "received" })
        }
...
export const acceptConnection = async (req, res) => {
    try {
        ...
        if (!connection) {
            return res.status(400).json({ message: "connection deos not exist" })
        }
...
export const getConnectionRequests = async (req, res) => {
    try {
        ...
        return res.status(500).josn({ message: `getConnectionRequests ${error}` })
    }
}
